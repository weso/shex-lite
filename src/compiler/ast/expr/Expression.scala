 ...
 *
 * When an expression is added to the ast it is mandatory to add its is and as methods, that way it is much
 * simple and faster to check if the ast is semantically correct or not and also to access fields.
 *
