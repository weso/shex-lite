...
class CGJava04ConstructorGenerator(ccontext: CompilationContext, stringBuilder: StringBuilder)
  ...
  override def visit(expr: ConstraintBlockTripleExpr, className: String): Unit = {
    ...
    for (i <- 0 to fields.size - 1) {
      fields(i).asConstraintTripleExpr.constraint
        .accept(this, fields(i).asConstraintTripleExpr.cardinality.asCardinalityExpr.max.toString)
      stringBuilder
        .append(fields(i).asConstraintTripleExpr.property.asCallPrefixExpr.argument.toLowerCase)
      if (i != fields.size - 1) { stringBuilder.append(", ") }
    }
