 ...
 */
case class Warn(node: NodeWithPosition, message: String, ttype: CompilerEventType) extends CompilationEvent {
  ...
  override def toPrintableString: String = {
    ...
    sb.append(s"${Console.YELLOW}error[${ttype.getCode}]: ${Console.RESET}${ttype.getDescription}")
    sb.append("\n")
    sb.append(s"--> ${node.getPosition.filename}:${node.getPosition.line}:${node.getPosition.column}")
...
object Warn {
  ...
  val SchemaWithoutDirectMapping = new CompilerEventType {
    ...
    override def getDescription: String = "schema includes constraints without a direct mapping to target language"
  }
