 ...
 */
case class Err(node: NodeWithPosition, message: String, ttype: CompilerEventType) extends CompilationEvent {
  ...
  override def toPrintableString: String = {
    ...
    sb.append(s"${Console.RED}error[${ttype.getCode}]: ${Console.RESET}${ttype.getDescription}")
    sb.append("\n")
