...
/**
  * A Constraint Node IRI Expression indicated that a node constraint is of
  * type IRI. Does not add more functionality
  * than the classification of the node constraint type.
  *
  * @author Guillermo Facundo Colunga
  */
class ConstraintNodeIRIExpr(position: Position, tokenRange: Interval, content: String)
    extends ConstraintNodeExpr(position, tokenRange, content) {

  // Override default methods to indicate that this is a Constraint Node IRI
  // Expression.
