...
class JavaCodeGenTest extends AnyFunSuite with BeforeAndAfter {
  ...
  private[this] def generateTestCases(): Unit = {
    ...
    for (file <- correctFiles) {
      test(
        ...
        val sources = compilationResult.getIndividualResults
          .toList(0)
          .getGeneratedSources
          .get(ShExLCompilerTargetLanguage.Java)
          .get
          .toList
    ...
    for (file <- incorrectFiles) {
      test(
        ...
        compilationResult.getIndividualResults.last.printWarnings
        println(
          compilationResult.getIndividualResults
  ...
  test(
    ...
    compileResult.getIndividualResults.foreach(result =>
      result.getGeneratedSources
        .get(ShExLCompilerTargetLanguage.Java)
        .get
        .foreach(source => println(source.getSource))
    )
    val expected =
    ...
    val results = compileResult.getIndividualResults.head.getGeneratedSources
      .get(ShExLCompilerTargetLanguage.Java)
      .get
